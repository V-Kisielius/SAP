...
def main(
    ...
    models_directory = 'weights_and_data/real_model/real_model_new/0/'
    ...
    print(f"Данные загружены. Наблюдаемый ряд: {len(observed_series)} точек")
    print(f"Коэффициент нормализации: {normalization_factor:.2f}")
    ...
    print(f"Предсказания выполнены. Форма массива: {predictions.shape}")
    ...
    if need_plot:
        print("Создаем график предсказаний...")
        plot_predictions(
            observed_series=observed_series,
            theoretical_series=theoretical_series,
            normalization_factor=normalization_factor,
